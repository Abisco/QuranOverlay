class QuranVersesController < ApplicationController
    ...
    def index
        if params[:verse_number] && params[:surah_number]
            @quran_verse = QuranVerse.where("verse_id >= ? AND verse_id <= ? AND surah_id = ?", params[:verse_number].to_i - 1,  params[:verse_number].to_i + 1, params[:surah_number].to_i).order("verse_id ASC")
